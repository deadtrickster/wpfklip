...
namespace WpfKlip.Core.Win
{
    ...
    /// </summary>
    class ShellThumbnailsService
    {
        internal static Bitmap GetThumbOrIcon(string path)
        {
            ShellThumbnailsService ShellThumbnailsService = new ShellThumbnailsService();
            try
            {
                var thumb = ShellThumbnailsService.GetThumbNail(path);
                return thumb;
            }
            catch
            {
                /*if (Environment.OSVersion.Version.Major > 5)
                    return FileToIconConverter.GetFileIcon(path, FileToIconConverter.IconSize.jumbo).ToBitmap();
                else
                    return FileToIconConverter.GetFileIcon(path, FileToIconConverter.IconSize.extraLarge).ToBitmap();*/
